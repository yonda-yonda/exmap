...
// https://github.com/yonda-yonda/terminator-geojson/tree/master

export class NoConvergenceError extends Error {
...
const falsePositionMethod = (
  ...
  const { eps, maxIter } = Object.assign(
  ...
  if (count >= maxIter) throw new NoConvergenceError();
  return c;
};
...
const getLatitude = (
  ...
  const lat = falsePositionMethod(
    (x) => {
      return (
        Math.sin(delta) * Math.sin(x) +
        Math.cos(delta) * Math.cos(x) * Math.cos(hourAngle) -
        Math.sin(elevation)
      );
    },
    [-Math.PI / 2, Math.PI / 2],
  ...
  if (lat < -Math.PI / 2) return -Math.PI / 2;
  if (Math.PI / 2 < lat) return Math.PI / 2;
...
export const calculate = (options?: {
  date?: Date;
  division?: number;
  elevationDegree?: number | "civil" | "nautical" | "astronomical";
}): Feature<Polygon> => {
  const { date, division, elevationDegree } = Object.assign(
    { date: new Date(), division: 360, elevationDegree: 0 },
    options
  );

  const elevation =
    typeof elevationDegree === "string"
      ? getTwilight(elevationDegree)
      : (elevationDegree * Math.PI) / 180;
  if (Math.PI / 4 < Math.abs(elevation)) throw new RangeError();
